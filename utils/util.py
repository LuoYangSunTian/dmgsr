...
def anisotropic_gaussian_kernel(batch, kernel_size, covar):
    ...
    # xx: torch.Size([32, 21, 21])
    # [-10,-9...10]、[-10,-9...10]
    yy = ax.repeat_interleave(kernel_size).view(1, kernel_size, kernel_size).expand(batch, -1, -1)
    # yy: torch.Size([32, 21, 21])
    # [-10,-10,10...-10]、[-9,-9,..-9]、...[10,10,10,..,10]
    xy = torch.stack([xx, yy], -1).view(batch, -1, 2)
    # xy: torch.Size([32, 441, 2]),
    # torch.stack([b, c], -1):torch.Size([32, 21, 21, 2])
...
def isotropic_gaussian_kernel(batch, kernel_size, sigma):
    # sigma:torch.Size([32]),0.2-2之间的随机数
    ax = torch.arange(kernel_size).float().cuda() - kernel_size // 2
    #   # ax = [-10,-9,...,0,1,...10]
    xx = ax.repeat(kernel_size).view(1, kernel_size, kernel_size).expand(batch, -1, -1)
    # xx: torch.Size([32, 21, 21])
    ...
    # 将kernel看做32张图的对应21*21卷积核大小的的权重
    return kernel / kernel.sum([1, 2], keepdim=True)
...
def random_isotropic_gaussian_kernel(batch=1, kernel_size=21, sig_min=0.2, sig_max=4.0):
    ...
    print("[DEBUG]-----Shape of k:", k.shape)
    return k, x
# 这部分为修改部分
...
def random_gaussian_kernel(batch, kernel_size=21, blur_type='iso_gaussian', sig_min=0.2, sig_max=4.0, lambda_min=0.2,
                           lambda_max=4.0):
...
class BatchBlur(nn.Module):
    ...
    def forward(self, input, kernel):
        ...
        if len(kernel.size()) == 2:
            ...
            # input_CBHW代表输入图像的大小(minibatch，in_channels，H，W)
            # 32*21*21的权重b_kernels， kernel.contiguous()深拷贝功能，改变后与原来的值无关
            kernel = kernel.contiguous().view((B, 1, self.kernel_size, self.kernel_size))  # 32 * 1 * 21* 21

            kernel = kernel.repeat(1, C, 1, 1).view((B * C, 1, self.kernel_size, self.kernel_size))
            # 32 * c*2 * 21 * 21
            #  kernel代表卷积核的大小(out_channels，in_channe/groups，H，W)
    ...
    def __call__(self, hr_tensor, random=True):
        with torch.no_grad():
            ...
            # gaussian blur + downsampling
                ...
                B, N, C, H, W = hr_tensor.size()
                # 返回32*21*21的权重b_kernels
                b_kernels, sigma_kernel = self.gen_kernel(B, random)  # B degradations

                # blur
                # B, N, C, H, W = hr_tensor.size(),N=2，即一张图片返回两个patch，用于对比损失
            ...
            if self.mode == 'bicubic':
                ...
                lr_blured = hr_blured.view(-1, C, H // self.scale, self.scale, W // self.scale, self.scale)[:, :, :, 0,
                            :, 0]
            ...
            return lr_blured.view(B, N, C, H // int(self.scale), W // int(self.scale)), b_kernels, sigma_kernel
