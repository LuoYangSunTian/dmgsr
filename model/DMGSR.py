...
from moco.builder_DMGSR import MoCo
...
class OCAB(nn.Module):
    ...
    def forward(self, x, x_size, rpi):
        ...
        # print(q_windows.size())
        kv_windows = self.unfold(kv)  # b, c*w*w, nw
        # print(kv_windows.size())
...
class WAttention(nn.Module):
    ...
    def forward(self, x, kv, rpi, mask=None):
        """
        Args:
            x: input features with shape of (num_windows*b, n, c)
            mask: (0/-inf) mask with shape of (num_windows, Wh*Ww, Wh*Ww) or None
            :param kv: degrade information
        """
        b_, n, c = x.shape
        k = kv.reshape(b_, n, 1, self.num_heads, c // self.num_heads).permute(2, 0, 3, 1, 4)
        k = k[0]
...
class DGFT(nn.Module):
    ...
    def forward(self, x, x_size, rpi_sa, kv):
        h, w = x_size
        b, _, c = x.shape
        shortcut = x
        x = self.norm1(x)
        ...
        x_k = self.conv(torch.cat((x.permute(0, 3, 1, 2), k.permute(0, 3, 1, 2)), dim=1))
        # Conv_X
        conv_x = self.conv_block(x_k)
        conv_x = conv_x.permute(0, 2, 3, 1).contiguous().view(b, h * w, c)
        ...
        # W-MSA/SW-MSA (to be compatible for testing on images whose shapes are the multiple of window size
        attn_windows = self.attn(x_windows, k_windows, rpi=rpi_sa, mask=None)
...
class DMGSR(nn.Module):
    def __init__(self, args,
        ...
        self.con = nn.Sequential(
            nn.Conv2d(192, 96, 1, 1, 0, bias=False),
    ...
    def forward_features(self, x, kv):
        ...
        for i in range(self.num_layers):
            ...
            x = self.cons[i](x, x_size, params['rpi_sa'], kv)
        x = self.last_layer(x, x_size, params)
    ...
    def forward(self, x, k_v):
        x_size = [x.shape[2], x.shape[3]]
        k_v = F.interpolate(k_v, x_size, mode='bicubic')
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x = self.conv_first(x)
            x1 = self.con(torch.cat((x, k_v), dim=1))
            # deep feature extraction
            x = self.conv_after_body(self.forward_features(x1, k_v)) + x
            # reconstruction
...
class LayerNorm2d(nn.Module):
    ...
    def forward(self, x):
        ...
        std = (x - mean).pow(2).mean(dim=[2, 3], keepdim=True).add(self.eps).sqrt()
        x = (x - mean) / std
        if self.affine:
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        fea = self.E(x)
        f = self.e(fea).squeeze(-1).squeeze(-1)
        out = self.mlp(f)
        return fea, out
...
class BlindSR(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            x_query, x_key, kernel = x  # b, c, h, w
